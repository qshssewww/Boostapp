...
function get_newboostapp_domain()
{
    return isDevEnviroment() ? 'https://devnew.boostapp.co.il' : 'https://new.boostapp.co.il';
}

function get_loginboostapp_domain()
{
    return isDevEnviroment() ? 'https://devlogin.boostapp.co.il' : 'https://login.boostapp.co.il';
}

function get_apiboostapp_domain()
{
    return isDevEnviroment() ? 'https://devapi.boostapp.co.il' : 'https://api.boostapp.co.il';
}

function get_paymentboostapp_domain()
{
    return isDevEnviroment() ? 'https://devpayment.boostapp.co.il' : 'https://payment.boostapp.co.il';
}

function get_appboostapp_domain()
{
    return isDevEnviroment() ? 'https://devapp.boostapp.co.il' : 'https://app.boostapp.co.il';
}

function get_boostapp_domain()
{
    return isDevEnviroment() ? 'https://dev.boostapp.co.il' : 'https://www.boostapp.co.il';
}

function get_joinboostapp_domain()
{
    return isDevEnviroment() ? 'https://devjoin.boostapp.co.il' : 'https://join.boostapp.co.il';
}

function get_wpboostapp_domain()
{
    return isDevEnviroment() ? 'https://wp.boostapp.co.il' : 'https://wp.boostapp.co.il';
}

function get_cmsboostapp_domain()
{
    return isDevEnviromentSsh() ? 'https://devcmsboostapp.inmanage.com' : 'https://cmsboostapp.inmanage.com';
}
function get_letts_domain()
{
    return isDevEnviromentSsh() ? 'https://devexplore.boostapp.co.il' : 'https://letts.co.il';
}
...
{
    $dev_domains = array(
        'devlogin.boostapp.co.il',
        'devlogin2.boostapp.co.il',
        'devlogin3.boostapp.co.il',
        'devapp.boostapp.co.il',
        'devapp2.boostapp.co.il',
        'devapi.boostapp.co.il',
        'devpayment.boostapp.co.il',
        'devboostapp.co.il',
        'devnew.boostapp.co.il',
        'devjoin.boostapp.co.il',
        'devcheckin.boostapp.co.il',
        'devwp.boostapp.co.il',
        'devcmsboostapp.inmanage.com',
        'devexplore.boostapp.co.il',
        'devbiz.pinkapp.co.il',
        'devbiz1.pinkapp.co.il',
        'devbiz2.pinkapp.co.il',
        'devbiz3.pinkapp.co.il'
//        'localhost:8000'
    );
    $is_dev = false;

    if (!isset($_SERVER['HTTP_HOST']) || in_array($_SERVER['HTTP_HOST'], $dev_domains)) {
        $is_dev = true;
    }
    return $is_dev;
}
...
function lang($key, $params = [])
{
    ...
    $Translations = Hazzard\Translation\TranslationManager::getInstance();
 ...
 */
function transDbVal($val)
{

    require_once __DIR__ . '/../../../office/Classes/TranslationDbValues.php';

    if (!isset($_SESSION['lang'])) {
        $_SESSION['lang'] = 'he';
    }

    $DbTranslations = TranslationDbValues::getInstance();

    $key = $DbTranslations->getKeyFromValue($val);
    if (empty($key)) {
        return $val;
    }
    return lang($key);
 ...
 * @return bool
 */
function isDevEnviromentSsh()
{
    if (isset($_SERVER['LOGNAME']) && strpos($_SERVER['LOGNAME'], 'dev') !== false) {
        return true;
    } else {
        return false;
    }
 ...
 * @return void
 */
function CreateLogMovement($LogContent, $ClientId)
{
    try {
        ...
        // if user authenticated -> take CompanyNum from user, if not -> take CompanyNum from client
        if (!Auth::guest()) {
            $LogUserId = Auth::user()->id;
            $CompanyNum = Auth::user()->CompanyNum;
        } else {
            if (!$client) {
                throw new LogicException('Wrong ClientId and non-authenticated user while creating log movement. Content: ' . $LogContent);
            }
        ...
        // replace {clientProfileLink} with the link to the client profile
        $ClientLink = '<a href="ClientProfile.php?u=' . $ClientId . '" >' . $clientName . '</a>';
        $LogContent = str_replace("{clientProfileLink}", $ClientLink, $LogContent);

        DB::table('boostapp.log')->insert([
            'UserId' => $LogUserId,
            'Text' => $LogContent,
            'Dates' => $LogDateTime,
            'ClientId' => $ClientId,
