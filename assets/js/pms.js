// Private Messages
BeePOS.addContact = function (userId) {
	$.post(BeePOS.options.ajaxUrl, {action: 'addContact', id: userId});
	$('[data-contact-id="'+userId+'"]').fadeOut(200, function() { $(this).remove() });
};

BeePOS.confirmContact = function (userId) {
	$.post(BeePOS.options.ajaxUrl, {action: 'confirmContact', id: userId});
	$('[data-contact-id="'+userId+'"]').addClass('contact-confirmed');
};

BeePOS.removeContact = function (userId) {
	$.post(BeePOS.options.ajaxUrl, {action: 'removeContact', id: userId});

	$('[data-contact-id="'+userId+'"]').fadeOut(200, function() { $(this).remove() });
};

jQuery(function($) {
	var _log = function (xhr) { BeePOS.log((xhr.responseJSON && xhr.responseJSON.error) 
	...
	var _realtime = BeePOS.options.pms.realtime,
		_delay = parseInt(BeePOS.options.pms.delay) * 1000;
		...
		$.ajax({
			...
			if (!response.message.length)
				conversationList.find('.list-group').append(
					'<li class="no-messages">'+BeePOS.trans('no_messages')+'</li>'
				);
	...
	var markAllAsRead = function (e) {
		...
		$.post(BeePOS.options.ajaxUrl, {action: 'markAllAsRead'}, function() {
			loadCoversationList(true);
			checkNotifications();
		}, 'json');
	};
	...
	var openConversation = function(e) {
		...
		BeePOS.ajaxFormCb.sendMessage = function(message, form) {
	...
	var deleteMessages = function (e) {
		...
		$.post(BeePOS.options.ajaxUrl, {action: 'deleteMessage'}, function() {
			checkNotifications();
		}, 'json');
	...
	var searchContact = function (e) {
		...
		$.get(BeePOS.options.ajaxUrl, {action: 'searchContact', user: value}, function (response) {
