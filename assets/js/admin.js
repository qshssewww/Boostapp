jQuery(function ($) {   
    var trans = function (m) { return BeePOS.trans(m) };

    var profileUrl =function (id, name) { 
        return '<a href="'+BeePOS.options.baseUrl+'/profile.php?u='+id+'" target="_blank">'+name+'</a>';
    }

    var usersDataTable, messagesDataTable;

    // Users DataTable
    BeePOS.admin.usersDT = function () {
			...
			columns: [
			    ...
			    null,
			    {
                    ...
                    render: function (d, t, r) {
                        ...
                        '<a href="javascript:BeePOS.admin.composeEmail(\''+r[2]+'\')" title="'+trans('send_email')+'"><span class="glyphicon glyphicon-envelope"></span></a> '+
                        '<a href="javascript:BeePOS.admin.deleteUser('+r[0]+', \''+(r[1]||r[2])+'\')" title="'+trans('delete_user')+'"><span class="glyphicon glyphicon-trash"></span></a>';
                    }
                }
			]
		});
        ...
        */
        ...
        form.on('submit', function (e) {
            ...
            BeePOS.ajaxFormCb.deleteUsers = function () {
                usersDataTable.draw();
                modal.modal('hide');
            };
        });
	...
	// Delete User
	BeePOS.admin.deleteUser = function (userId, username) {
        ...
        // Register ajax form callback
        BeePOS.ajaxFormCb.deleteUser = function () {
            usersDataTable.draw();
            modal.modal('hide');
        };
	};
     ...
     // Messages DataTable
    BeePOS.admin.messagesDT = function () {
            ...
            columns: [
                ...
                null,
                {
                   render: function (d, t, r) {
                        ...
                        '<a href="javascript:BeePOS.admin.deleteConversation('+r[5]+', \''+r[2]+'\')" title="'+trans('delete_conversation')+'"><span class="glyphicon glyphicon-trash"></span></a>';
                    }
                }
            ],
        ...
        $('#messages_form').on('submit', function (e) {
            ...
            BeePOS.ajaxFormCb.deleteConversations = function () {
    ...
    BeePOS.admin.deleteConversation = function (id, username) {
        var modal = $('#deleteConversationModal');
        modal.find('input[name="user_id"]').val(id);
        modal.find('.user').text(username);
        modal.modal('show');

        BeePOS.ajaxFormCb.deleteConversation = function () {
            messagesDataTable.draw();
            modal.modal('hide');
        };
    };
    ...
    BeePOS.admin.searchContact = function() {
        var div = $('.search-user'),
        ...
        div.on('input', '.search', function (e) {
            ...
            $.get(BeePOS.options.ajaxUrl, {action: 'searchContact', admin: true, user: value}, function (response) {
