    ...
    for ($j = 0; $j < $iterations; $j++) {
        ...
        foreach ($GetSettings_splice as $GetSetting) {
            ...
            $GetClasses = DB::table('classstudio_date')
                ...
                ->where('Status', '=', '0')
                ->whereNotNull('meetingTemplateId')
                ->whereNotIn('MeetingStatus', [MeetingStatus::COMPLETED, MeetingStatus::CANCELED, MeetingStatus::DIDNT_ATTEND])
                ->where('StartDate', '=', $ThisDate)
                ->where('ClassType', '=', '1')
                ->get();

            foreach ($GetClasses as $GetClasse) {
                $ClassStudioDateRegular = DB::table('classstudio_dateregular')
                    ->where('CompanyNum', '=', $GetSetting->CompanyNum)
                    ->where('GroupNumber', '=', $GetClasse->GroupNumber)
                    ->where('Status', 0)
                    ->first();

                if(!$ClassStudioDateRegular){
                    continue;
                }
                ...
                $ClassInfo = DB::table('classstudio_date')
                    ...
                    ->first();
                $ClassCountsNum = DB::table('classstudio_date')
                    ->where('CompanyNum', '=', $GetSetting->CompanyNum)
                    ->where('Status', '=', '0')
                    ->whereNotNull('meetingTemplateId')
                    ->whereNotIn('MeetingStatus', [MeetingStatus::COMPLETED, MeetingStatus::CANCELED, MeetingStatus::DIDNT_ATTEND])
                ...
                if (!empty($ClassInfo->ClassRepeatType)) {
                    ...
                    if ($ClassRepeatType == '1') {
                        $ClassRepeatTypeText = 'day';
                    } elseif ($ClassRepeatType == '2') {
                        $ClassRepeatTypeText = 'week';
                    } elseif ($ClassRepeatType == '3') {
                        $ClassRepeatTypeText = 'month';
                    }
                }
                ...
                if($ClassStudioDateRegular->EndDate != null && $ClassStudioDateRegular->EndDate < $EndDates){
                    $EndDates = $ClassStudioDateRegular->EndDate;
                }
                ...
                foreach ($dateArr as $key => $value) {
                    ...
                    if (!$CheckFloor) {
                        ...
                        $AddClassDesk = DB::table('classstudio_date')->insertGetId(array(
                            ...
                            'MeetingStatus' => MeetingStatus::ORDERED,
                            'PreparationTimeMinutes' => $ClassInfo->PreparationTimeMinutes,
                            'purchaseAmount' => $ClassInfo->purchaseAmount,
                        ));
                        ...
                        foreach ($GetClients as $GetClient) {
                            ...
                            if ($CheckClient && in_array($CheckClient->Status, [0,2])) {
                                ...
                                if ($RegularClassType == '1' || ($RegularClassType == '2' && $ActDates == '0')) {
                                    ...
                                    $StatusJson .= '{"Dates": "' . $Dates . '", "UserId": "", "Status": "' . $Status . '", "StatusTitle": "' . $StatusTitle . '", "UserName": ""}';
                                    ...
                                    if ($ActivityInfo) {
                                        ...
                                        if (isset($MeetingSettings)) {
                                            ...
                                            if ($ClientActivitiesId == 0) {
                                                /** @var ClassStudioDate $ClassStudioDateNew */
                                                $ClassStudioDateNew = ClassStudioDate::find($AddClassDesk);
                                                $assignRes = ClientActivities::assignMembership([
                                                    "clientId" => $GetClient->ClientId,
                                                    "itemId" => $itemId,
                                                    "itemPrice" => $ClassStudioDateNew->purchaseAmount, //get updated price from last purchase
                                                    "activityName" => $ClassStudioDateNew->getSingleItemName(),
                                                    "companyNum" => $CompanyNum,
                                                    "isForMeeting" => 1,
                                                    "isDisplayed" => 0,
                                                    "fromCron" => true,
                                            ...
                                            $cancellationPolicyId =  $GetClassClientInfos->MeetingCancellationPolicy ?? null;
                                        }
