...
$pageTitle = lang('reports_notifications_log');
...
CreateLogMovement(lang('log_of_notifications'), '0');
...
$Items = DB::table('appnotification')->where('CompanyNum','=',$CompanyNum)->where('Type','=','3')->whereBetween('Date', array($StartDate, $EndDate))->orderBy('id', 'DESC')->get();
...
window.location.href = 'LogNotification.php?Dates='+value;
...
$(document).ready(function(){
   ...
   var categoriesDataTable =   $('#categories').dataTable( {
		...
		//{extend: 'copy', text: 'העתק <i class="fa fa-clipboard" aria-hidden="true"></i>', className: 'btn btn-info'},
			{extend: 'excelHtml5',  text: 'Excel <i class="fas fa-file-excel" aria-hidden="true"></i>', filename: '<?php echo lang('reports_notifications_log') ?>', className: 'btn btn-dark'},
			{extend: 'csvHtml5', text: 'CSV <i class="fas fa-file-code" aria-hidden="true"></i>', filename: '<?php echo lang('reports_notifications_log') ?>' , className: 'btn btn-dark'},
           // 'pdfHtml5'
		<?php endif ?>
			
        ],
	   
	   		ajax: { url: 'LogNotificationPost.php?<?php echo @$_SERVER['QUERY_STRING']; ?>', },
...
<div class="col-md-10 col-sm-12">	
    ...
    <i class="fas fa-filter"></i> <b><?php echo lang('reports_notifications_log') ?></b>
 	</div>    
  	<div class="card-body">       
