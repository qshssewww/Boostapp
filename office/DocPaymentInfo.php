...
require_once '../app/initcron.php'; 
			...
			101 => "אין אישור מחברה אשראי לעבודה עם כרטיס אשראי מסוג זה",	
...
$NewExcessTwo = $TempInfo->Amount-$GetAmountNow;
    
if ($CashValue>$TempInfo->Amount && $GetAmountNow=='0'){
     
$Excess = $CashValue-$TempInfo->Amount; 
$TotalCash = $CashValue-$Excess;  
    
}
else if ($CashValue>$NewExcessTwo) {

$Excess = $CashValue-$NewExcessTwo;	
$TotalCash = $CashValue-$Excess;   
...
$ClientInfo = DB::table('client')->where('id','=', $TempInfo->ClientId)->where('CompanyNum','=', $CompanyNum)->first();	     
...
$tashTypeDB = '5';     
            ...
            $CheckClient = DB::table('client')->where('id','=',$TempInfo->ClientId)->where('CompanyNum', $CompanyNum)->first();    
            ...
            $BrandName = 'כרטיס ויזה כאל מסוג '.@$CardType[$Brand];    
            }       
            else if ($Issuer=='3'){
...
array('UserId' => $UserId, 'Text' => $TextResults, 'ClientId' => $TempInfo->ClientId, 'CompanyNum' => $CompanyNum, 'Status' => $CCode));     
...
<div id="info" class="alertb alert-dark" dir="rtl">
 סה"כ לתשלום <span style='font-weight:bold; color:black; padding-right:5px;'><span id='TotalFinal'><?php echo number_format((float)$TempInfo->Amount, 2, '.', ''); ?></span> ₪</span>
</div>
