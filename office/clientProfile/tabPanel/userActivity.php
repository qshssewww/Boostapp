<div class="card spacebottom">
            ...
            <div class="card-body text-end">
                  ...
                  if (!$isRandomClient) {
                    ...
                    if (!in_array($SettingsInfo->beta, [1])) {
                        if (@$Supplier->Status!='2') {
                            ?>
                            <a href="#" data-ip-modal="#AddActivityPopup" name="AddActivity" class="btn btn-primary text-white"><?php echo lang('customer_card_add_membership') ?>
                            </a>
                        <?php } elseif($checkTryLimit < 5) {
                            $CheckLeadActivity = DB::table('client_activities')->where('Department', '=','3')->where('CompanyNum', $CompanyNum)->where('ClientId', $Supplier->id)->count();
                            if (@$CheckLeadActivity<='3'){
                                ?>
                                <a href="#" data-ip-modal="#AddActivityPopup" name="AddActivity" class="btn btn-primary text-white"><?php echo lang('customer_card_add_membership') ?>
                                </a>
                            <?php } } }
              ...
              <?php if (@$Supplier->PayClientId == '0') {} else {
...
?>
              ...
              <?php } } ?>
              <div class="row"  >
                <table class="table dt-responsive text-start wrap ActivityTable"   cellspacing="0" width="100%" id="ActivityTable">
                  <thead class="thead-dark">
                    <tr>
                      ...
                      <th  style="text-align:start;"><?php echo lang('membership_type_single') ?>
                      </th>
                      <th  style="text-align:start;"><?php echo lang('price') ?>
                      </th>
                      <th  style="text-align:start;"><?php echo lang('customer_card_balance') ?>
                      </th>
                      ...
                      <th  style="text-align:start;"><?php echo lang('classes') ?>
                      </th>
                      <th  style="text-align:start;">% <?php echo lang('usage') ?>
                      </th>
                    </tr>
                  </thead>
                  <tbody>
...
?>
          ...
          <td
              <?php echo $LineThrough; ?> >
          <?php echo number_format($MemberShipClient->ItemPrice*$MemberShipClientQuantity, 2); ?>
          </td>
        <td
            <?php echo $LineThrough; ?> >
        <?php echo $MemberShipClient->BalanceMoney; ?>
        </td>
      <td
          <?php echo $LineThrough; ?> >
    ...
    <td
        <?php echo $LineThrough; ?> >
    <?php if ($MemberShipClient->FirstDateStatus=='0') { echo with(new DateTime($MemberShipClient->StartDate))->format('d/m/Y'); } else { echo lang('short_first_class_date'); } ?>
    </td>
  ...
  <a href='javascript:LogActivity("<?php echo $MemberShipClient->id; ?>");'>
    <span class="<?php echo $TrueDateColor; ?>">
      <?php if ($MemberShipClient->FirstDateStatus=='0') { echo with(new DateTime($MemberShipClient->VaildDate))->format('d/m/Y'); } else { echo ''; } ?>
    </span>
  </a>
  <?php } ?>
  </td>
...
<a href='javascript:LogActivity("<?php echo $MemberShipClient->id; ?>");'>
  <span class="<?php echo $TrueBalanceValueColor; ?> text-center unicode-plaintext" id="ClientProfileTRDiv_Card<?php echo $MemberShipClient->id ?>">
...
<?php endif; ?>
        ...
        <?php } ?>
<td>
  <?php if (($MemberShipClient->Department == '1' && $MemberShipClient->Status != '3' && $MemberShipClient->Status != '2') || ($MemberShipClient->Department == '2' && $MemberShipClient->Status != '3' && $MemberShipClient->Status != '2') || ($MemberShipClient->Department == '3' && $MemberShipClient->Status != '3' && $MemberShipClient->Status != '2')) { ?>
  <div class="progress">
    <div class="progress-bar bg-success progress-bar-striped active" role="progressbar" aria-valuenow="<?php echo $completed; ?>" aria-valuemin="0" aria-valuemax="100" style="width: <?php echo $completed; ?>%;" data-toggle="tooltip" data-placement="top" title='<?php echo $completed; ?>%'>
    </div>
  </div>
  <?php } else if ($MemberShipClient->Status=='2'){ echo lang('membership_canceled'); } ?>
</td>
</tr>
