...
<?php CreateLogMovement('נכנס לדוח לידים לפי נציג','0'); ?>
...
$(document).ready(function(){
   ...
   var categoriesDataTable =   $('#categories').dataTable( {
		...
		//{extend: 'copy', text: 'העתק <i class="fa fa-clipboard" aria-hidden="true"></i>', className: 'btn btn-info'},
			{extend: 'excelHtml5',  text: 'Excel <i class="fas fa-file-excel" aria-hidden="true"></i>', filename: 'דו״ח לידים לנציג', className: 'btn btn-dark',exportOptions: {}},
			{extend: 'csvHtml5', text: 'CSV <i class="fas fa-file-code" aria-hidden="true"></i>', filename: 'דו״ח לידים לנציג' , className: 'btn btn-dark',exportOptions: {}},
           // 'pdfHtml5'
		 <?php endif ?>
...
<i class="fas fa-chart-pie fa-fw"></i> דו״ח לידים לנציג
</div>
</h3>
</div>
...
<nav aria-label="breadcrumb" dir="rtl">
  ...
  <li class="breadcrumb-item active" aria-current="page">דו״ח לידים לנציג</li>
  </ol>  
</nav>    
...
<div class="card spacebottom">
      <div class="card-header text-right" dir="rtl"><i class="fas fa-chart-pie fa-fw"></i><strong> דו״ח לידים לנציג</strong></div>    
  <div class="card-body">       
...
<thead>
          <tr class="bg-dark text-white">
            <th style="text-align:right;">שם הנציג</th>
            <?php 
...
<tbody>
<?php
$Agents = DB::table('users')->where('CompanyNum', '=', $CompanyNum)->where('role_id', '!=', '1')->orderBy('display_name', 'ASC')->get();
	foreach ($Agents as $Agent) {
$PipeLineCountTotal = DB::table('pipeline')->where('UserId', '=', @$Agent->id)->where('CompanyNum', '=', $CompanyNum)->groupBy('UserId')->count();
$PipeLineCountSuccess = DB::table('pipeline')->where('PipeId', '=', $CompanyNum.'98')->where('UserId', '=', @$Agent->id)->where('CompanyNum', '=', $CompanyNum)->groupBy('UserId')->count();
$PipeLineCountFaild = DB::table('pipeline')->where('PipeId', '=', $CompanyNum.'99')->where('UserId', '=', @$Agent->id)->where('CompanyNum', '=', $CompanyNum)->groupBy('UserId')->count();

?>
<tr>
<td><a href="AgentProfile.php?u=<?php echo @$Agent->id; ?>"><?php echo @$Agent->display_name; ?></a></td>
            <?php 
			$LeadsStstuss = DB::table('leadstatus')->where('Status', '=', '0')->orderBy('Sort', 'ASC')->get();
			foreach ($LeadsStstuss as $LeadsStstus) {
				$PipeLineCount = DB::table('pipeline')->where('PipeId', '=', @$LeadsStstus->id)->where('UserId', '=', @$Agent->id)->where('CompanyNum', '=', $CompanyNum)->groupBy('UserId')->count();
