...
//סטטוסים למסמכים
		...
		if (@$Docs->Status == '0') {DB::table('docs')->where('id', $Docs->id)->where('CompanyNum',  $CompanyNum)->update(array('Status' => '1'));}
//סטטוסים למסמכים
...
$DocKindName = 'מקור'; // Makor / Copy / etc...
...
if (strlen(@$ClientPhone) == '9') {$ChunkAfter = '2';} else {$ChunkAfter = '3';} //לבדוק כמה ספרות המספר ולהוסיף מקף בהתאם לכמות הספרות
$DocHtml .= '
<tr class="ClientTable">
...
if ($DocPayment->TypePayment == '1') {$DocPaymentNotes = '';}
elseif ($DocPayment->TypePayment == '2') {$DocPaymentNotes = 'מספר המחאה: '.@$DocPayment->CheckNumber.', קוד בנק: '.@$DocPayment->CheckBankCode.', סניף: '.@$DocPayment->CheckBankSnif.', מספר חשבון: '.@$DocPayment->CheckBank;}
elseif ($DocPayment->TypePayment == '3') {$DocPaymentNotes = @$DocPayment->BrandName.' המסתיים ב-'.@$DocPayment->L4digit.' תשלום '.@$DocPayment->Payments.' מתוך  '.$PaymentNum.' '.array_search(@$DocPayment->tashType, $TashType).', מס׳ אישור: '.@$DocPayment->ACode;}
elseif ($DocPayment->TypePayment == '4') {$DocPaymentNotes = 'מספר אסמכתא: '.@$DocPayment->BankNumber;}
elseif ($DocPayment->TypePayment == '5') {$DocPaymentNotes = '';}
elseif ($DocPayment->TypePayment == '6') {$DocPaymentNotes = '';}
elseif ($DocPayment->TypePayment == '7') {$DocPaymentNotes = '';}
elseif ($DocPayment->TypePayment == '8') {$DocPaymentNotes = '';}
elseif ($DocPayment->TypePayment == '9') {$DocPaymentNotes = '';}
...
$mpdf->Output($CompanyName.' - '.$DocTypeName.' '.$DocId.'.pdf', 'I');
//==============================================================
//==============================================================
//==============================================================
}
