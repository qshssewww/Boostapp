<?php
redirect_to(__DIR__.'/Cal.php');
exit;

require_once '../app/init.php'; 
$pageTitle = lang('task_type_manage');
...
$Items = DB::table('caltype')->where('CompanyNum','=', $CompanyNum)->orderBy('Status', 'ASC')->get();
...
$(document).ready(function(){
   ...
   var categoriesDataTable =   $('#categories').dataTable( {
		...
		//{extend: 'copy', text: 'העתק <i class="fa fa-clipboard" aria-hidden="true"></i>', className: 'btn btn-info'},
			{extend: 'excelHtml5',  text: 'Excel <i class="fas fa-file-excel" aria-hidden="true"></i>', filename: '<?php echo lang('tasks_type') ?>', className: 'btn btn-dark',exportOptions: {columns: [ 0, 1, 2 ]}},
			{extend: 'csvHtml5', text: 'CSV <i class="fas fa-file-code" aria-hidden="true"></i>', filename: '<?php echo lang('tasks_type') ?>', className: 'btn btn-dark',exportOptions: {columns: [ 0, 1, 2 ]}},
           // 'pdfHtml5'
		<?php endif ?>			
        ],
...
<i class="fas fa-thumbtack fa-fw"></i> סוגי משימות <span style="color:#48AD42;"><?php //echo $resultcount; ?> </span>
</div>
</h3>
</div>
...
<?php //if (Auth::userCan('137')): ?>    
<a href="#" data-ip-modal="#CalTypePopup" class="btn btn-success btn-block" name="Items"  ><i class="fas fa-plus-circle fa-fw"></i> הוסף סוג משימה חדשה</a>
<?php //endif ?>    
</div>
...
<nav aria-label="breadcrumb" >
  ...
  <li class="breadcrumb-item active" aria-current="page">סוגי משימות</li>
  </ol>  
</nav>     -->

<?php if (Auth::userCan('137')): ?>    
<a href="javascript:;" data-ip-modal="#CalTypePopup" class="floating-plus-btn d-flex bg-primary" title="<?php echo lang('new_task_type') ?>">
...
<div class="col-md-10 col-sm-12">	
    ...
    <div class="card-header text-start d-flex justify-content-between" >
        <div>
    <i class="fas fa-thumbtack"></i> <b><?php echo lang('tasks_type') ?> <span style="color:#48AD42;"><?php echo $resultcount; ?> </span></b></div>  
 	</div>    
  	<div class="card-body">       
...
?>        
        ...
        <?php if (Auth::userCan('137')): ?>     
        <td class="text-start"><a class="btn btn-success btn-sm" style="color: #FFFFFF !important;" href='javascript:UpdateCalType("<?php echo $Item->id; ?>");'><?php echo lang('edit_task_type') ?></a></td>
        <?php endif ?>      
        </tr>
...
<!-- DepartmentsPopup -->
	<div class="ip-modal" id="CalTypePopup">
		<div class="ip-modal-dialog" <?php //_e('main.rtl') ?>>
			<div class="ip-modal-content text-start">
				...
				<h4 class="ip-modal-title"><?php echo lang('new_task_type') ?></h4>
                
				</div>
				<div class="ip-modal-body" >
<form action="AddCalType"  class="ajax-form clearfix">
                <div class="form-group" >
                <label><?php echo lang('task_type') ?></label>
                <input type="text" name="Type" id="Type" class="form-control" placeholder="<?php echo lang('task_type') ?>">
                </div>     
    
     			<div class="form-group">
                <label><?php echo lang('background_color') ?></label>
                <div id="SetDocBacksPreviews" style="background-color:#b79bf7 ;width:50px;height:10px;display:inline-block;"></div>
                <select class="form-control" name="DocsBackgroundColor" id="DocsBackgroundsColors" onchange="dsfsds()">
...
<!-- Edit DepartmentsPopup -->
	<div class="ip-modal" id="EditCalTypePopup" tabindex="-1">
		<div class="ip-modal-dialog" <?php //_e('main.rtl') ?>>
			<div class="ip-modal-content text-start">
				<div class="ip-modal-header d-flex justify-content-between" >
				<h4 class="ip-modal-title"><?php echo lang('a_edit_task_type') ?></h4>
                <a class="ip-close" title="Close" data-dismiss="modal" aria-hidden="true"  >&times;</a>
                
				</div>
				<div class="ip-modal-body" >
<form action="EditCalType"  class="ajax-form clearfix">
...
$(function() {
			...
			// Header setup
			$('#CalTypePopup').imgPicker({
			});
			// Header setup
			$('#EditCalTypePopup').imgPicker({
			});
	
});
    
function dsfsds() {
    var x = document.getElementById("DocsBackgroundsColors").value;
    document.getElementById("SetDocBacksPreviews").style.backgroundColor = x;
}       
</script>
