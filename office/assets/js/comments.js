jQuery(function($) {
	function _log (xhr) { 
		Beesoft.log((xhr.responseJSON && xhr.responseJSON.error) ? xhr.responseJSON.error : xhr.responseText) 
	}
	...
	Comments.prototype = {
		...
		bindEvents: function () {
			...
			this.find('.show-more').on('click', function() {
				$(this).attr('data-loading-text', Beesoft.trans('loading'));
		...
		attachEvents: function (comment) {
			comment.find('.comment-content')
				...
				.on('keyup', 'textarea', commentLengthCounter)
				.on('submit', '.ajax-form', Beesoft.ajaxForm)
		...
		trash: function (e) {
			...
			$.post(Beesoft.options.ajaxUrl, {action: 'trash_comment', id: id});
		},
		...
		vote: function (e) {
			...
			$.post(Beesoft.options.ajaxUrl, {action: 'vote_comment', id: $votes.attr('data-comment'), type: type});
		},

		renderCount: function(count) {
			if (count) this.count = count;

			this.find('.comment-count').text(this.count + Beesoft.trans(this.count == 1 ? 'comment' : 'comments'));
		},
	...
	Beesoft.ajaxFormCb.addComment = function(comment, form) {
	...
	Beesoft.ajaxFormCb.updateComment = function(comment) {
