// Private Messages
Beesoft.addContact = function (userId) {
	$.post(Beesoft.options.ajaxUrl, {action: 'addContact', id: userId});
	$('[data-contact-id="'+userId+'"]').fadeOut(200, function() { $(this).remove() });
};

Beesoft.confirmContact = function (userId) {
	$.post(Beesoft.options.ajaxUrl, {action: 'confirmContact', id: userId});
	$('[data-contact-id="'+userId+'"]').addClass('contact-confirmed');
};

Beesoft.removeContact = function (userId) {
	$.post(Beesoft.options.ajaxUrl, {action: 'removeContact', id: userId});

	$('[data-contact-id="'+userId+'"]').fadeOut(200, function() { $(this).remove() });
};

jQuery(function($) {
	var _log = function (xhr) { Beesoft.log((xhr.responseJSON && xhr.responseJSON.error) 
	...
	var _realtime = Beesoft.options.pms.realtime,
		_delay = parseInt(Beesoft.options.pms.delay) * 1000;
		...
		$.ajax({
			...
			if (!response.message.length)
				conversationList.find('.list-group').append(
					'<li class="no-messages">'+Beesoft.trans('no_messages')+'</li>'
				);
	...
	var markAllAsRead = function (e) {
		...
		$.post(Beesoft.options.ajaxUrl, {action: 'markAllAsRead'}, function() {
			loadCoversationList(true);
			checkNotifications();
		}, 'json');
	};
	...
	var openConversation = function(e) {
		...
		Beesoft.ajaxFormCb.sendMessage = function(message, form) {
			Beesoft.alert(null, null, form);
	...
	var deleteMessages = function (e) {
		...
		$.post(Beesoft.options.ajaxUrl, {action: 'deleteMessage'}, function() {
			checkNotifications();
		}, 'json');
	...
	var searchContact = function (e) {
		...
		$.get(Beesoft.options.ajaxUrl, {action: 'searchContact', user: value}, function (response) {
	...
	var showAllContacts = function (e) {
		...
		$.get(Beesoft.options.ajaxUrl, {action: 'all_contacts'}, function (response) {
			list.html('');
	...
	// Open messages modal
	Beesoft.openPMS = function (userId) {
