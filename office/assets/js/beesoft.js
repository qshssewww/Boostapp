window.Beesoft = {
	...
	trans: function (message) {
        return (Beesoft.options.lang[message] || message.toString());
    },
	...
	log: function (message) {
		if (Beesoft.options.debug) console.log('Server response:', message);
	},
	...
	ajaxForm: function (e) {
		...
		btn.attr('data-loading-text', Beesoft.trans('loading'));
		
		btn.button('loading'); Beesoft.alert(null, null, form);
		...
		.done(function (response) {
			if (response.success) {
				if (Beesoft.ajaxFormCb[action]) 
					Beesoft.ajaxFormCb[action](response.message, form);
			} else { 
				Beesoft.alert(response.message || Beesoft.trans('error'), -1, form);
				Beesoft.log(response);
			}
		})
		// If the ajax fails show a generic error to the user and log the actual error.
		.fail(function (jqXHR) {
			Beesoft.alert(Beesoft.trans('error'), -1, form);
			Beesoft.log((jqXHR.responseJSON && jqXHR.responseJSON.error) ? 
    ...
    logout: function() {
    	$.post(Beesoft.options.ajaxUrl, {action: 'logout'}, function() {
...
jQuery(function ($) {
	...
	$('.ajax-form').on('submit', Beesoft.ajaxForm);
