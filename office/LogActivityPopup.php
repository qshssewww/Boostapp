...
<div class="col-md-6">	 
<span class="text-center font-weight-bold">שיעורים מוזמנים עתידיים: <?php echo @$GetClientClassCounts; ?></span>
 </div>      
...
<select id="ShowStatusBalance" class="form-control form-control-sm">
  <option value="All">הצג הכל
  <option value="Nikov" selected>הצג ניקובים בלבד
</select>
 </div>     
</div>
<hr>
...
$class_day = $classStudioDateInfo->Day ?? $ClassLog->Day;
...
<span class="text-center font-weight-bold">סיבת ביטול מנוי</span><br>
...
<?php
        ...
        if(empty(@$UserName->display_name)) {
                ...
                $operation = 'פעולה אוטומטית:: ';          
        }
        else {
                $operation = 'פעולה ידנית:: ';
        ...
        echo $ClassNumber = '<div class="alertb alert-light">'.$operation.' '.$BalanceText.' <span dir="ltr">'.$val['ClassNumber'].'</span> שיעורים. <br>'.$val['Reason'].'<br>
...
<?php if ($Items->Department=='1' && $Items->StudioStartDateLog!='' || $Items->Department=='2' && $Items->TrueDate!='' && $Items->StudioStartDateLog!=''){ ?>
<span class="text-center font-weight-bold">לוג שינוי תאריך התחלת מנוי</span>
...
<?php
        ...
        echo $ClassNumber = '<div class="alertb alert-light">שינוי תאריך התחלה למנוי <span dir="ltr">'.with(new DateTime($val['TrueDate']))->format('d/m/Y').' <br>במקום '.with(new DateTime($val['StudioStartDate']))->format('d/m/Y').'</span> <br>'.$val['Reason'].'<br>
        <span style="font-size: 13px;"> עודכן ע"י '.@$UserName->display_name.' בתאריך: '.with(new DateTime($val['Dates']))->format('d/m/Y H:i').'<span></div><hr>'; 
        }                                                                  
?>  
...
<span class="text-center font-weight-bold">לוג שינוי תוקף מנוי</span>
...
<?php
        ...
        echo $ClassNumber = '<div class="alertb alert-light">שינוי תוקף למנוי <span dir="ltr">'.with(new DateTime($val['TrueDate']))->format('d/m/Y').'</span> <br>'.$val['Reason'].'<br>
        <span style="font-size: 13px;"> עודכן ע"י '.@$UserName->display_name.' בתאריך: '.with(new DateTime($val['Dates']))->format('d/m/Y H:i').'<span></div><hr>'; 
        }                                                                  
?>  
