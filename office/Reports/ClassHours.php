...
$pageTitle = lang('reports_payroll_guide');
...
<?php CreateLogMovement('fas fa-chart-pie',lang('report_payroll_log '),'0');
...
$(document).ready(function(){
   ...
   var categoriesDataTable =   $('#categories').dataTable( {
            ...
            "ordering": false,
            ajax: {
   			url: 'ClassHoursPost.php?StartDateWeek=<?php echo $StartDateWeek; ?>&EndDateWeek=<?php echo $EndDateWeek; ?>&Class=<?php echo $Class; ?>&Guide=<?php echo $Guide; ?>',
		    type: 'POST',
    		},
	       // autoWidth: true,
          //  "order": [[ 0, "ASC" ]],
...
<div class="card spacebottom">
      <div class="card-header text-start d-flex justify-content-between">
        <div><i class="fas fa-user-tag"></i><strong> <?php echo lang('reports_payroll_guide') ?></strong></div> 
        </div>   
  <div class="card-body">       
...
<form name="ThisForm" method="get">
    ...
    <?php echo lang('reports_set_time_notice') ?>
    </div>
 ...
 <div class="form-group row text-start" >
    <div class="col-sm-12">
      <button type="submit" class="btn btn-dark text-white" id="MakeFile" name="MakeFile" value="MakeFile"><?php echo lang('extract_report_payroll') ?></button>
    </div>
  </div>	
</form>
...
<thead>
          <tr class="bg-dark text-white">
            ...
            <th style="text-align:start;"><?php echo lang('begin_time') ?></th>   
            <th style="text-align:start;"><?php echo lang('finish_time') ?></th>
            <th style="text-align:start;"><?php echo lang('total_hours') ?></th>
            <th style="text-align:start;"><?php echo lang('table_total_to_pay') ?></th>  
...
var selected = $(this).val();
  ...
  if(selected != null)
  {
    if(selected.indexOf('BA999')>=0){
      $(this).val('BA999').select2( {theme:"bootstrap", placeholder: "<?php echo lang('choose_class') ?>", 'language':"he", dir: "rtl" } );
    }
  }
...
var selected = $(this).val();
  ...
  {
    if(selected.indexOf('BA999')>=0){
      $(this).val('BA999').select2( {theme:"bootstrap", placeholder: "<?php echo lang('select_coach') ?>", 'language':"he", dir: "rtl" } );
    }
  }
