                                    ...
                                    <table class="table table-hover dt-responsive text-right display wrap" id="dataTable" dir="rtl" cellspacing="0" width="100%">
                                        ...
                                        <thead>
                                            <tr class="bg-dark text-white">
                                                ...
                                                <th style="text-align:right;">מקור הגעה</th>
                                                <th style="text-align:right;">סטאטוס</th>
                                                <th style="text-align:right;">מתעניין בתחום</th>
                                                <th style="text-align:right;">נציג מטפל</th>
                                            </tr>
                                            <tr class="bg-white text-black filterHeader">
                                                <th></th>
                                                <th style="text-align:right;">
                                                        <input name="dateRange" type="text" class="form-control" placeholder="חפש מתאריך">
                                                </th>
                                                <th style="text-align:right;"><input name="name" type="text" class="form-control" placeholder="חפש לפי שם"></th>
                                                <th style="text-align:right;"><input name="email" type="text" class="form-control" placeholder="חפש לפי דואר אלקטרוני"></th>
                                                <th style="text-align:right;"><input name="phone" type="text" class="form-control" placeholder="חפש לפי טלפון"></th>
                                                <th style="text-align:right;">
                                                ...
                                                <th style="text-align:right;"><input name="intrested" type="text" class="form-control" placeholder="חפש לפי תחום"></th>
                                                <th style="text-align:right;">
...
.datepicker.dropdown-menu {right:auto}
        ...
        (function($, BeePOS){
            $(document).ready(function(){
                ...
                var fields = {
                    ...
                    dateFrom: $("input[name='dateFrom']", filter),
                    dateTo: $("input[name='dateTo']", filter),
                    name: $("input[name='name']", filter),
                    email: $("input[name='email']", filter),
                    phone: $("input[name='phone']", filter),
                    branch: $("select[name='branch']", filter),
                    source: $("select[name='source']", filter),
                    status: $("select[name='status']", filter),
                    intrested: $("input[name='intrested']", filter),
                    agent: $("select[name='agent']", filter)  
                }
    ...
    // set status
    $.get('../rest/?type=report&method=pipeline&data=statusList', function(data){
        try{
            data = JSON.parse(data);
            var statuses = data.items || [];
            for (let i = 0; i < statuses.length; i++) {
                            fields.status.append(jQuery('<option>', {value: statuses[i].value, text: statuses[i].value}))  
                       }
        }catch(e){}
    })
    ...
    $.get('../rest/?type=report&method=branches', function(data){
                ...
                $.get('../rest/?type=report&method=pipelineSources', function(data){
                    try {
                       data = JSON.parse(data); 
                       var sources = data.items || [];
                       for (let i = 0; i < sources.length; i++) {
                            fields.source.append(jQuery('<option>', {value: sources[i].source, text: sources[i].source}))  
                       }
...
//                    scrollCollapse: true,
                    ...
                    buttons: [
                        ...
                        <?php if (Auth::userCan('98')): ?>    
                            {extend: 'excelHtml5',  text: 'Excel <i class="fas fa-file-excel" aria-hidden="true"></i>', filename: 'דו״ח מתעניינים', className: 'btn btn-dark',exportOptions: {modifier: {
                  page: 'all',
                  search: 'none'                  
                }}},
			                {extend: 'csvHtml5', text: 'CSV <i class="fas fa-file-code" aria-hidden="true"></i>', filename: 'דו״ח מתעניינים' , className: 'btn btn-dark',exportOptions: {modifier: {
                  page: 'all', 
                  search: 'none'                  
                }}},
             ...
             .css( 'direction', 'rtl' )
                    ...
                    ajax:{
                        ...
                        data: function(d){
                            ...
                            d.filter = {
                                ...
                                intrested: fields.intrested.val(),
                                agent: fields.agent.val(),
                    ...
                    pageLength: 100,
                    lengthChange: true,
                    lengthMenu: [ 10, 25, 50, 75, 100, 150, 200, 250, 300, 500,1000,2000,3000,4000 ],
                    columnDefs: [
                        {
                            'targets': [0],
                            'checkboxes': {
                ...
                table.dataTable(settings).on('xhr.dt', function ( e, settings, json, xhr ) {
                    ...
                    json.data = json.items.map(function(x){
                        ...
                        data.push(x.branch || '');
                        data.push(x.inputSource || '');
                        data.push(x.status || '');
                        data.push(x.intrested || '');
                        data.push(x.agent || '');
                        return data
                    });
                    // json.draw = 1;
