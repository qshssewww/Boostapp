...
$pageTitle = lang('reports_fixed_payroll_title');
...
<?php CreateLogMovement('fas fa-chart-pie',lang('fixed_payroll_log '),'0');
...
$(document).ready(function(){
   ...
   var categoriesDataTable =   $('#categories').dataTable( {
            ...
            "ordering": false,
            ajax: {
   			url: 'ClassClientFixPost.php?StartDateWeek=<?php echo $StartDateWeek; ?>&EndDateWeek=<?php echo $EndDateWeek; ?>&Class=<?php echo $Class; ?>&Guide=<?php echo $Guide; ?>',
		    type: 'POST',
    		},
	       // autoWidth: true,
          //  "order": [[ 0, "ASC" ]],
		...
		//{extend: 'copy', text: 'העתק <i class="fa fa-clipboard" aria-hidden="true"></i>', className: 'btn btn-info'},
			{extend: 'excelHtml5',  text: 'Excel <i class="fas fa-file-excel" aria-hidden="true"></i>', filename: '<?php echo lang('reports_fixed_payroll_title') ?>', className: 'btn btn-dark',exportOptions: {}},
			{extend: 'csvHtml5', text: 'CSV <i class="fas fa-file-code" aria-hidden="true"></i>', filename: '<?php echo lang('reports_fixed_payroll_title') ?>' , className: 'btn btn-dark',exportOptions: {}},
           // 'pdfHtml5'
		  <?php endif ?>
...
<i class="fas fa-user-tag"></i> דו״ח שכר - סכום קבוע לשיעור
</div>
</h3>
</div>
...
<!-- <nav aria-label="breadcrumb" >
  ...
  <li class="breadcrumb-item active" aria-current="page">דו״ח שכר - סכום קבוע לשיעור</li>
  </ol>  
</nav>     -->
...
<div class="card spacebottom">
      <div class="card-header text-start"><i class="fas fa-user-tag"></i><strong> <?php echo lang('reports_fixed_payroll_title') ?></strong></div>    
  <div class="card-body">       
...
<form name="ThisForm" method="get">
    ...
    <?php echo lang('set_date_range_class') ?>
    </div>
    ...
    <option value=""></option>         
    <?php
    $Activities = DB::table('users')->where('CompanyNum', '=', $CompanyNum)->where('status', '1')->where('Coach', '1')->orderBy('display_name', 'ASC')->get();
 ...
 <div class="form-group row text-start">
    <div class="col-sm-12">
      <button type="submit" class="btn btn-dark text-white" id="MakeFile" name="MakeFile" value="MakeFile"><?php echo lang('reports_set_fixed_notice') ?></button>
    </div>
  </div>	
</form>
...
<thead>
          <tr class="bg-dark text-white">
            ...
            <th style="text-align:start;"><?php echo lang('time_of_a_class') ?></th>   
