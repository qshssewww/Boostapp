    ...
    $.get('../rest/?type=report&method=branches', function(data){
                ...
                table.dataTable(settings).on('xhr.dt', function ( e, settings, json, xhr ) {
                    ...
                    json.data = json.items.map(function(x){
                        ...
                        data.push(x.email || '');
                        if(x.mobile && x.mobile.indexOf('0') == 0){
                            data.push('<a href="tel:+'+x.mobile.replace('0', '972').replace(/\D/g,'')+'">'+(x.mobile || '') +'</a>');   
                        }else{
                            data.push(x.mobile || '');
                        }
                        data.push(x.gender || '');
