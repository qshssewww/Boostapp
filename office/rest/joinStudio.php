...
SELECT 
    ...
    DATE_FORMAT(client.Dates, '%d/%m/%Y') as displayDate,
    ...
    joinedClient.amount as joinedClientLast30days,
    ROUND( (totalClient.amount/joinedClient.amount) , 2) as percentJoin,
    totalJoined.amount as totalJoined,
...
LEFT JOIN 
    (SELECT COUNT(*) as amount FROM `client` WHERE client.CompanyNum = $rest->CompanyNum and client.Status IN (0) and client.Dates BETWEEN (CURDATE() - INTERVAL 30 DAY) and CURDATE()) as joinedClient On 1 = 1
LEFT JOIN 
    (SELECT COUNT(*) as amount FROM `client` WHERE client.CompanyNum = $rest->CompanyNum and client.Status IN (0)) as totalJoined On 1 = 1
LEFT JOIN brands
    ON brands.CompanyNum = $rest->CompanyNum AND brands.id = client.Brands
WHERE
    client.CompanyNum = $rest->CompanyNum and client.Status IN (0)
";
