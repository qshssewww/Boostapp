...
if (!isset($_REQUEST["startDate"])) $_REQUEST["startDate"] = date("Y-m-01");
...
?>    
    ...
    <span >    
    <i class="fas fa-times fa-fw"></i> <b>ניהול מתעניינים - אבודים</b>
    </span> 
    <div class="d-flex ">   
     <span class="mie-6 mb-6" >
	<input id="dateRangesf" type="text" placeholder="חפש לפי טווח תאריכים" class="dateRangesf form-control" autocomplete="off">		
	</span>  	    
...
<table class="table table-hover dt-responsive text-start display wrap" id="LeadsFails"  cellspacing="0" width="100%">
		<thead>
			<tr class="bg-dark text-white">
                ...
                <th class="text-start">סיבת כישלון</th>
...
<?php 
if ($TypeRanges=='0'){
    $LeadsFails = DB::table('pipeline')->where('CompanyNum','=', $CompanyNum)->where('StatusFilter', '=', '2')->whereBetween('Dates', array($StartDate, $EndDate))->orderBy('Dates', 'DESC')->get();
}
else {
    $LeadsFails = DB::table('pipeline')->where('CompanyNum','=', $CompanyNum)->where('StatusFilter', '=', '2')->whereBetween('ConvertDate', array($StartDate, $EndDate))->orderBy('ConvertDate', 'DESC')->get();
}

//$LeadsFails = DB::table('pipeline')->where('CompanyNum','=', $CompanyNum)->where('StatusFilter', '=', '2')->get();
$LeadsFailsShow = [];
foreach ($LeadsFails as $fail){
    $ClientLeadStatus = DB::table('leadstatus')->where('CompanyNum','=', $CompanyNum)->where('id','=',$fail->PipeId)->first();
    if(!empty($ClientLeadStatus) && $ClientLeadStatus->Act == 2){
        array_push($LeadsFailsShow,$fail);
    }
}


foreach ($LeadsFails as $LeadsOpen) {
...
if ($LeadsOpen->ReasonsId=='0'){
$LeadsReasons = '';    
}   
else {    
$PipeReasonsInfo = DB::table('pipereasons')->where('CompanyNum','=', $CompanyNum)->where('id', '=', $LeadsOpen->ReasonsId)->first();     
$LeadsReasons = $PipeReasonsInfo->Title.', ';  
}
...
?>            
        ...
        <td><?php echo @$LeadsReasons; ?> <?php echo $LeadsOpen->FreeText; ?></td>    
        <td><?php echo with(new DateTime(@$LeadsOpen->Dates))->format('d/m/Y H:i'); ?></td>
        <td><?php echo with(new DateTime(@$LeadsOpen->ConvertDate))->format('d/m/Y H:i'); ?></td>    
        </tr>    
...
<?php } ?>            
	...
	<tfoot>
            <tr>
                ...
                <th><select id="table-filterBrands-Fails" class="form-control">
...
<th><select id="table-filterPipeLine-Fails" class="form-control">
<option value="">הכל</option>
<?php 
...
<th><select id="table-filterPipeLineSource-Fails" class="form-control">
<option value="">הכל</option>
<?php 
...
<th><select id="table-filterPipeLineAgent-Fails" class="form-control">
<option value="">הכל</option>
<?php 
...
<?php } ?>                    
        ...
        var direction = false ;
              ...
              direction = true ;
        		...
        		var dateRange = jQuery('#dateRangesf');
						dateRange.daterangepicker({
                            ...
                            $.get('LeadsList/fails.php?TypeRanges='+TypeRanges+'&startDate='+moment(d.startDate).format('YYYY-MM-DD')+'&endDate='+moment(d.endDate).format('YYYY-MM-DD'), function(data){
                            $('#nav-fails').html(data);
                            });
...
$(document).ready(function(){
	 ...
	 $('#LeadsFails tfoot th span').each( function () {
        var title = $(this).text();
   ...
   var categoriesDataTable =   $('#LeadsFails').dataTable( {
...
//	        pageLength: 5000,
      ...
      'select': {
		...
		//{extend: 'copy', text: 'העתק <i class="fa fa-clipboard" aria-hidden="true"></i>', className: 'btn btn-info'},
			{extend: 'excelHtml5',  text: 'Excel <i class="fas fa-file-excel" aria-hidden="true"></i>', filename: 'מתעניינים אבודים', className: 'btn btn-dark'},
			{extend: 'csvHtml5', text: 'CSV <i class="fas fa-file-code" aria-hidden="true"></i>', filename: 'מתעניינים אבודים' , className: 'btn btn-dark'},
...
//		order: [[2, 'DESC']]
		    ...
		    var table = $('#LeadsFails').DataTable();  
...
$('#LeadsFails tfoot tr').insertAfter($('#LeadsFails thead tr'));   
        
$('#table-filterBrands-Fails').on('change', function(){
table.column('4').search("^" + this.value + "$", true, false, true).draw();   
});    
    
$('#table-filterPipeLine-Fails').on('change', function(){
table.column('5').search(this.value).draw();   
});   
    
$('#table-filterPipeLineSource-Fails').on('change', function(){
table.column('6').search(this.value).draw();   
});   
    
$('#table-filterPipeLineAgent-Fails').on('change', function(){
