...
try {
    $paymentResult = PaymentService::chargeClient($data, true);
...
<?php } else { ?>
    <script>
        $(document).ready(function () {
            let act = '<?= $Act ?>';
            if(act == 3) {
                $('#AddDocsRefoundClient').submit();
            }
        });
        Swal.fire({
            text: lang('action_done'),
            icon: 'success'
        });
    </script>
<?php } ?>
...
<table class="table" dir="rtl">
    ...
    foreach ($TempsPayments as $TempsPayment) {
        ...
        <tr>
            ...
            <td>
                <button class="btn btn-outline-danger btn-sm btn-rounded CancelPaymentsRefound" type="button" name="CancelPayments"
                        data-ajax="<?php echo $TempsPayment->id; ?>"><?php echo lang('action_cacnel') ?></button>
            </td>
        </tr>
...
<script>
    ...
    $('#FinalinvoicenumRefound').val('<?php echo number_format((float)$MoreAmount, 2, '.', ''); ?>');
    $('#CancelDocs_FinalinvoicenumRefound').val('<?php echo number_format((float)$TrueFinalinvoicenum, 2, '.', ''); ?>');

    <?php
    if (@$CheckRefresh == '2') {
    ?>
    $('#CreditRefounddiv input').val('');
    $('#CreditRefounddiv select').prop('selectedIndex', 0);
    $('#CahshdRefoundiv input').val('');
    $('#CheckRefounddiv input').val('');
    $('#BankRefounddiv input').val('');

    <?php } ?>

    $('#CreditValueButtonRefound').attr("disabled", true);
    $('.CloseCheckBoxPaymentRefound').attr("disabled", true);

    <?php if ($GetAmount + $GetExcess > '0'){ ?>
    $('#CancelDocButtonRefound').attr("disabled", false);
    <?php } else if ($GetAmount + $GetExcess == '0' || $GetAmount + $GetExcess == '0.00') { ?>
    $('.CloseCheckBoxPaymentRefound').attr("disabled", false);
    <?php } ?>
    ...
    $('#CheckDateRefound').val('<?php echo $CheckDate; ?>');
    $('#CheckNumberRefound').val('<?php echo $CheckNumber; ?>');
    $('#CheckValueRefound').val('');
    ...
    $('#CreditRefounddiv input').val('');
    $('#CreditRefounddiv select').prop('selectedIndex', 0);
    $('#CreditRefounddiv #CreditOptionTokenRefound').prop('selectedIndex', 0).trigger('change');
    <?php } ?>
    $('#CahshRefounddiv input').val('');
    $('#CheckRefounddiv input').val('');
    $('#BankRefounddiv input').val('');
    ...
    $('.CancelPaymentsRefound').click(function () {
        $("#meTestRefound").trigger("click");
        var CancelPayments_TempsListsId = $(this).data("ajax");
        $('#CancelPayments_TempsListsIdRefound').val(CancelPayments_TempsListsId);
    });
