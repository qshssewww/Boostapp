...
<div class="alertb alert-danger" id="ClientWatingListText" style="display: none;">	
 ...
 <div class="row">
 <div class="col-md-12">	
 <?php echo lang('trainees_booked_permanently') ?>:
...
<th style="text-align:right;"><?php echo lang('class_customer_num') ?></th>    
<th style="text-align:right;"><?php echo lang('class_table_name') ?></th>
<th style="text-align:right;"><?php echo lang('phone') ?></th> 
<th style="text-align:right;"><?php echo lang('status') ?></th>
<th style="text-align:right;"><?php echo lang('class_set_start_date') ?></th>     
<th style="text-align:right;"><?php echo lang('class_set_end_date') ?></th>     
</thead>

<tbody>
<?php 
$i= '1';
$Clients = DB::table('classstudio_dateregular')->where('GroupNumber', '=', $ClassInfo->GroupNumber)->where('CompanyNum', $CompanyNum)->orderBy('StatusType','DESC')->orderBy('id','ASC')->get(); 
foreach ($Clients as $Client) {

$ClientName = DB::table('client')->where('id', '=', $Client->ClientId)->where('CompanyNum', $CompanyNum)->first();

$StatusInfoColor = DB::table('class_status')->where('id', '=', $Client->StatusType)->first();     
...
<td class="align-middle"> <?php if ($Client->StatusType=='12'){ echo lang('setting_permanently'); } else { echo lang('permanent_waitlist'); } ?></td> 

<td class="align-middle"> <?php if ($Client->StartDate!=''){ echo with(new DateTime($Client->StartDate))->format('d/m/Y');  } else { } ?></td>      
    
<td class="align-middle"> <?php if ($Client->RegularClassType=='2'){ echo with(new DateTime($Client->EndDate))->format('d/m/Y');  } else { } ?></td>      
