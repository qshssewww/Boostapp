...
$pageTitle = lang('2_reports_no_response_waitlist');
require_once '../app/views/headernew.php';
?>


<?php if (Auth::check()):?>
<?php if (Auth::userCan('150')): ?>
<?php
CreateLogMovement(lang('reports_no_response_log'), '0');
?>
...
//                  table.ajax.reload();
                    window.location.href = "ClassNotificationReport.php?dateFrom="+moment(fields.date.data('daterangepicker').startDate._d).format('YYYY-MM-DD')+"&dateTo="+moment(fields.date.data('daterangepicker').endDate._d).format('YYYY-MM-DD');    
                    });
...
//	        pageLength: 5000,
		...
		//{extend: 'copy', text: 'העתק <i class="fa fa-clipboard" aria-hidden="true"></i>', className: 'btn btn-info'},
			{extend: 'excelHtml5',  text: 'Excel <i class="fas fa-file-excel" aria-hidden="true"></i>', filename: '<?php echo lang('2_reports_no_response_waitlist') ?>', className: 'btn btn-dark'},
...
<i class="fas fa-bell-slash"></i> דוח אי תגובה לרשימת המתנה</span>
</div>
</h3>
</div>
...
<nav aria-label="breadcrumb" dir="rtl">
  ...
  <li class="breadcrumb-item active">דוח אי תגובה לרשימת המתנה</li>
  </ol>  
</nav>     -->
...
<div class="col-md-10 col-sm-12 order-md-1">	
    ...
    <i class="fas fa-bell-slash"></i> <b><?php echo lang('2_reports_no_response_waitlist') ?></b>
 	</div>    
  	<div class="card-body">       
...
<table class="table table-hover dt-responsive text-right display wrap" id="categories" dir="rtl" cellspacing="0" width="100%">
		<thead >
			<tr class="bg-dark text-white">
				<th class="text-right"><?php echo lang('client') ?></th>
                <th class="text-right"><?php echo lang('telephone') ?></th>
                <th class="text-right"><?php echo lang('table_notification_date') ?></th>
                <th class="text-right"><?php echo lang('class_single') ?></th>
				<th class="text-right"><?php echo lang('table_class_date') ?></th>
				<th class="text-right lastborder"><?php echo lang('table_class_hour') ?></th>
			</tr>
...
<?php 
$CalTypes = DB::table('class_type')->where('CompanyNum', '=', $CompanyNum)->where('Status', '0')->get();
foreach ($CalTypes as $CalType) {                    
?>
<option><?php echo $CalType->Type; ?></option>                    
<?php } ?>                   
</select></th>
<th><span><?php echo lang('table_class_date') ?></span></th>
<th><span><?php echo lang('class_time') ?></span></th>
</tr>      
