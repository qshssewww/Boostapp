 ...
 <table class="table table-hover text-right" style="font-size:12px; font-weight:bold;" dir="rtl" id="Token">
  <thead >
          <tr style="background-color:#bce8f1;">
            ...
            <th align="right" style="text-align:right;">סיבת החזרה</th> 
...
$GetKevaInfos = DB::table('payment')->where('CompanyNum', '=', $CompanyNum)->where('Status', '=' , '2')->where('KevaId', '=' , $TokenId)->get();
...
?>    
    ...
    <td <?php echo $LineThrough; ?>><?php  echo with(new DateTime(@$GetKevaInfo->TryDate))->format('d/m/Y');  ?></td>    
    <td <?php echo $LineThrough; ?>><?php echo @$GetKevaInfo->NumTry; ?></td>           
    <td <?php echo $LineThrough; ?>><?php echo @$GetKevaInfo->Error; ?></td>
    <td <?php echo $LineThrough; ?>>
    <select name="StatusEvent" id="StatusEvent<?php echo $GetKevaInfo->id ?>" data-placeholder="בחר סטטוס" class="form-control" style="width:100%;">
<option value="<?php echo $GetKevaInfo->id ?>:2" <?php if ($GetKevaInfo->Status=='2'){ echo 'selected'; } else {} ?>>נכשל</option>
<option value="<?php echo $GetKevaInfo->id ?>:3" <?php if ($GetKevaInfo->Status=='3'){ echo 'selected'; } else {} ?>>חייב שוב עכשיו</option>
<option value="<?php echo $GetKevaInfo->id ?>:4" <?php if ($GetKevaInfo->Status=='4'){ echo 'selected'; } else {} ?>>סמן כחוב אבוד</option>
</select>	    
    </td>    
    </tr>
...
$("#StatusEvent<?php echo $GetKevaInfo->id ?>").change(function () {
var Acts = this.value;
$.ajax({
type: 'POST',    
